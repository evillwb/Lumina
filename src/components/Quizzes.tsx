...
export const Quizzes: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        const [qLogs, qTopics] = await Promise.all([
          getDocs(query(collection(db, 'users', user.uid, 'quizLogs'), orderBy('createdAt', 'desc'), limit(50))),
          getDocs(collection(db, 'users', user.uid, 'topics'))
        ]);
        
        setLogs(qLogs.docs.map(d => ({ id: d.id, ...d.data() } as QuizLog)));
        const fetchedTopics = qTopics.docs.map(d => ({ id: d.id, ...d.data() } as Topic));
  ...
  const startQuiz = async () => {
    ...
    try {
      ...
      const qArray = await generateQuizQuestions(topic.subject, topic.title, topic.notes, language, processedFiles, numQuestions);
      setQuizData(qArray);
  ...
  const submitAnswer = async (selectedAnswer: string) => {
    if (!quizData || !user || !selectedTopicId || selectedFeedback !== null || isProcessingAnswer) return;
    ...
    try {
      ...
      await updateDoc(topicRef, {
        masteryLevel: isCorrect ? Math.min(100, currMastery + 5) : Math.max(0, currMastery - 2),
           ...
           if (allCorrect) {
               ...
               getDoc(userRef).then(uDoc => {
                   if (uDoc.exists()) {
                       ...
                       alert("AMAZING! You conquered the Sunday Boss Battle and earned 500 Credits!");
                   }
               });
           } else {
               alert("Boss Battle finished, but you didn't get all of them correct. Try again next week!");
           }
       }
    }
  ...
  if (!user) {
    return (
      <div className="flex-1 p-8 flex items-center justify-center flex-col text-center">
        ...
        <p className="text-neutral-500 dark:text-neutral-400">Please sign in with Google to view quizzes.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 p-6 md:p-8 overflow-y-auto">
      <header className="mb-8">
        <h1 className="text-2xl font-semibold text-neutral-900 dark:text-white tracking-tight">{t('quizzes')}</h1>
        ...
        <div className="max-w-2xl">
          {!quizData && !quizLoading && (
            <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-6 shadow-sm">
                ...
                {topics.length === 0 ? (
                    <p className="text-sm text-amber-600 dark:text-amber-500">You need to add topics to your syllabus first.</p>
                ) : (
                    <div className="space-y-4">
                        <div>
                           <label className="text-xs text-neutral-500 font-bold uppercase mb-2 block">Select Topic</label>
                           <select 
                           ...
                           >
                              {topics.map(t => <option key={t.id} value={t.id}>{t.title} ({t.subject})</option>)}
                           </select>
                        </div>
                        
                        <div className="grid grid-cols-2 gap-4">
                           <div>
                              <label className="text-xs text-neutral-500 font-bold uppercase mb-2 block">Number of Questions</label>
                              <select 
                                 value={numQuestions}
                           ...
                           <div>
                              <label className="text-xs text-neutral-500 font-bold uppercase mb-2 block">Time Limit</label>
                              <select 
                                 value={timeLimitSecs}
                                 onChange={(e) => setTimeLimitSecs(parseInt(e.target.value))}
                                 className="w-full bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-lg p-3 text-neutral-900 dark:text-white text-sm focus:outline-none focus:border-blue-500"
                              >
                        ...
                        <div>
                           <label className="text-xs text-neutral-500 font-bold uppercase mb-2 block">Upload Context Document (Optional)</label>
                           <label className="border-2 border-dashed border-neutral-200 dark:border-neutral-800 rounded-xl p-4 flex flex-col items-center justify-center cursor-pointer hover:bg-neutral-50 dark:hover:bg-neutral-950 transition-colors">
                             ...
                             <span className="text-sm font-medium text-neutral-600 dark:text-neutral-300">Click to upload PDF or text file</span>
                        ...
                        {isSunday && (
                            <div className="mt-8 border-2 border-indigo-500 rounded-xl bg-indigo-50 dark:bg-indigo-500/10 p-6 flex flex-col justify-center items-center text-center">
                                <h3 className="text-lg font-bold text-indigo-700 dark:text-indigo-400 mb-2">🔥 Sunday Boss Battle! 🔥</h3>
                                <p className="text-sm text-indigo-600 dark:text-indigo-300 mb-4 opacity-90">Review all the questions you got wrong this week. Get a perfect score and win 500 Credits!</p>
                                <button 
          ...
          {quizLoading && (
                <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-12 shadow-sm flex flex-col items-center justify-center">
                  ...
                  <p className="text-sm text-neutral-500 dark:text-neutral-400">AI is analyzing your notes and generating a question...</p>
                </div>
          )}
            ...
            <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl shadow-sm p-6 overflow-hidden relative">
              ...
              <div className="flex justify-between items-center mb-6 mt-2">
                 <div className="text-[10px] font-bold text-blue-600 dark:text-blue-500 uppercase tracking-[0.2em]">
                    {examModeActive ? 'Exam Mode' : 'Practice Session'} • {currentQuestionIdx + 1}/{quizData.length}
                 </div>
                 {timeLeft !== null && (
                    <div className={`text-xs font-bold px-3 py-1 rounded-full ${timeLeft < 60 ? 'bg-rose-100 text-rose-600' : 'bg-neutral-100 text-neutral-600 dark:bg-neutral-800 dark:text-neutral-300'}`}>
              ...
              <h2 className="text-xl font-semibold text-neutral-900 dark:text-white mb-6 leading-tight">
                Review: <span className="text-blue-600 dark:text-blue-400">{topics.find(t => t.id === selectedTopicId)?.title}</span>
              </h2>
              ...
              {!quizFinished ? (
                 ...
                 <div className="space-y-6">
                    <div className="p-6 rounded-xl border bg-blue-50 border-blue-200 dark:bg-blue-500/10 dark:border-blue-500/20 text-center">
                       <h3 className="text-xl font-bold text-blue-800 dark:text-blue-400 mb-2">{examModeActive ? 'Exam Complete!' : 'Quiz Complete!'}</h3>
                       <p className="text-blue-600 dark:text-blue-500">
                          You scored {quizResults.filter(r => r.isCorrect).length} out of {quizData.length}
                          {examModeActive && <span className="block mt-2 font-bold">{(quizResults.filter(r => r.isCorrect).length / quizData.length * 100).toFixed(0)}% Final Score</span>}
                       </p>
                    </div>
                    ...
                    <button 
                       onClick={() => { setQuizData(null); setQuizResults([]); }}
      ...
      <div className="space-y-4 max-w-4xl">
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
           <div className="flex-1">
              <label className="text-xs text-neutral-500 font-bold uppercase mb-2 block">Filter by Topic</label>
              <select 
                 value={filterTopicId}
           ...
           <div className="flex-1">
              <label className="text-xs text-neutral-500 font-bold uppercase mb-2 block">Filter by Status</label>
              <select 
        ...
        {logs.filter(log => {
           if (filterTopicId !== 'all' && log.topicId !== filterTopicId) return false;
           if (filterCorrectness === 'correct' && !log.isCorrect) return false;
           if (filterCorrectness === 'incorrect' && log.isCorrect) return false;
           return true;
        }).length === 0 ? (
