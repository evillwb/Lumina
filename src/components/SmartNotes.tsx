...
export const SmartNotes: React.FC = () => {
  ...
  const [results, setResults] = useState<{ summaryBulletPoints: string[], keyTerms: string[] } | null>(null);
  ...
  const [processedFilesState, setProcessedFilesState] = useState<{ mimeType: string, data: string }[]>([]);
  ...
  const handleQuickQuestion = async (e: React.FormEvent) => {
    ...
    setQuestion('');
    setChatHistory(prev => [...prev, { role: 'user', text: userQ, timestamp: new Date() }]);
    setIsAsking(true);
    
    try {
      const summaryText = results.summaryBulletPoints.join('\n') + '\nTerms: ' + results.keyTerms.join(', ');
      const answer = await answerFollowUpQuestion(userQ, savedInputText, summaryText, processedFilesState);
      setChatHistory(prev => [...prev, { role: 'ai', text: answer, timestamp: new Date() }]);
    } catch (error) {
       console.error(error);
       setChatHistory(prev => [...prev, { role: 'ai', text: 'Sorry, I encountered an error while trying to answer that.', timestamp: new Date() }]);
    } finally {
      setIsAsking(false);
    }
  };
  ...
  const handleSaveToSyllabus = async () => {
    ...
    try {
      ...
      alert('Topic saved to your syllabus successfully!');
       ...
       handleFirestoreError(error, OperationType.CREATE, `users/${user.uid}/topics/${topicId}`);
    }
  };
    ...
    <div className="flex-1 flex flex-col md:flex-row gap-6 p-6 md:p-8 bg-neutral-50 dark:bg-[#09090b] overflow-hidden h-full">
      ...
      <div className="flex-1 flex flex-col h-full bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl shadow-sm overflow-hidden">
        <div className="p-6 border-b border-neutral-200 dark:border-neutral-800 flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-neutral-900 dark:text-white">Smart Notebook</h2>
            <p className="text-sm text-neutral-500">Paste your raw notes or upload files for an AI summary</p>
          </div>
        </div>
        <div className="flex-1 flex flex-col p-6 overflow-hidden">
          <div className="flex-1 flex flex-col bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-2xl overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500/50 focus-within:border-indigo-500 transition-all">
            ...
            {files.length > 0 && (
              <div className="px-4 pb-2 flex gap-2 flex-wrap">
                {files.map((f, i) => (
                  <div key={i} className="flex items-center gap-1.5 bg-neutral-200 dark:bg-neutral-800 px-3 py-1.5 rounded-lg text-sm text-neutral-700 dark:text-neutral-300">
                    <span className="truncate max-w-[150px] font-medium">{f.name}</span>
                    <button onClick={() => removeFile(i)} className="text-neutral-400 hover:text-neutral-600 dark:hover:text-white"><X className="w-3.5 h-3.5" /></button>
                  </div>
                ))}
              </div>
            )}
          ...
          <button
            ...
            disabled={(!inputText.trim() && files.length === 0) || isSummarizing}
      ...
      <div className="flex-1 flex flex-col h-full bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl shadow-sm overflow-hidden relative">
        ...
        <div className="p-6 border-b border-neutral-200 dark:border-neutral-800 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-neutral-900 dark:text-white">Results</h2>
              <p className="text-sm text-neutral-500">Structured summary & intelligence</p>
            </div>
          </div>
          ...
          {!isSummarizing && results && (
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-bold shadow-lg shadow-emerald-500/20 transition-transform active:scale-95"
              >
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 flex flex-col space-y-10">
          {isSummarizing && (
             <div className="space-y-8 animate-pulse bg-neutral-50 dark:bg-neutral-800/20 p-6 rounded-2xl border border-neutral-200 dark:border-neutral-800/50">
                <div className="flex items-center gap-3 mb-6">
                   ...
                   <p className="text-neutral-500 dark:text-neutral-400 font-medium tracking-tight">AI is analyzing your notes...</p>
                </div>
            ...
            <>
              ...
              <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500 bg-neutral-50 dark:bg-neutral-800/20 p-6 md:p-8 rounded-3xl border border-neutral-200 dark:border-neutral-800/50">
                 <div>
                    <div className="flex items-center gap-2 mb-6">
                      ...
                      <h3 className="text-xl font-bold text-neutral-900 dark:text-white tracking-tight">Summary</h3>
                    </div>
                 ...
                 <div>
                   <div className="flex items-center gap-2 mb-6">
                      ...
                      <h3 className="text-xl font-bold text-neutral-900 dark:text-white tracking-tight">Key Terms</h3>
                    </div>
                 ...
                 {generatedImage && (
                    <div className="mt-8 animate-in fade-in duration-500">
                        <div className="flex items-center gap-2 mb-4">
                          ...
                          <h3 className="text-xl font-bold text-neutral-900 dark:text-white tracking-tight">Visual Concept</h3>
                        </div>
                        <img 
                          src={generatedImage} 
              ...
              <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-6 shadow-sm flex flex-col">
                 <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="font-bold text-neutral-900 dark:text-white">Follow-up Questions</h3>
                 </div>
                 
                 <div className="space-y-4 mb-6">
                    {chatHistory.length === 0 && (
                      <p className="text-sm text-neutral-500 italic">Have a question about this topic? Ask below!</p>
                    )}
                      ...
                      <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        ...
                        {msg.role === 'user' && (
                          <div className="flex-shrink-0 w-8 h-8 rounded-full bg-neutral-200 dark:bg-neutral-800 flex items-center justify-center overflow-hidden border border-neutral-300 dark:border-neutral-700 ml-3 mt-1">
                            {user?.photoURL ? <img src={user.photoURL} alt="" className="w-full h-full object-cover" referrerPolicy="no-referrer" /> : <User className="w-4 h-4 text-neutral-500" />}
                          </div>
                        )}
                      </div>
                    ))}
                 ...
                 <form onSubmit={handleQuickQuestion} className="flex items-center gap-2 mt-auto relative">
                    <input 
                      type="text" 
                      value={question}
          ...
          {!isSummarizing && !results && (
            <div className="h-full flex flex-col items-center justify-center text-center text-neutral-400 px-6 mt-10">
               ...
               <h3 className="text-lg font-bold text-neutral-900 dark:text-white mb-2">Ready to Summarize</h3>
               <p className="text-sm max-w-[250px] leading-relaxed">Paste your text on the left and tap the summarize button to generate actionable study notes.</p>
            </div>
          )}
        </div>
      </div>
