...
export const CalendarView: React.FC = () => {
  ...
  const handleDrop = async (e: React.DragEvent<HTMLDivElement>, newDay: string) => {
    ...
    let newEndHour = parseInt(newStart.split(':')[0]) + (parseInt(block.endTime.split(':')[0]) - parseInt(block.startTime.split(':')[0]));
    if (newEndHour > 24) newEndHour = 24;
    const newEnd = newEndHour === 24 ? "23:59" : `${newEndHour.toString().padStart(2, '0')}:${block.endTime.split(':')[1]}`;
    ...
    try {
      ...
      setSchedule(s => s.map(b => b.id === draggedBlockId ? { ...b, day: newDay, startTime: newStart, endTime: newEnd } : b));
  ...
  const triggerEndBreakHighlight = () => {
     ...
     const nextBlock = schedule
       .filter(b => b.status === 'upcoming' && (b.day > today || (b.day === today && b.startTime >= curTime)))
       .sort((a, b) => {
  ...
  const fetchScheduleAndTopics = async () => {
    ...
    try {
      ...
      const bSnap = await getDocs(collection(db, 'users', user.uid, 'scheduleBlocks'));
      ...
      tSnap.docs.forEach(doc => { tMap[doc.id] = { id: doc.id, ...doc.data() } as Topic; });
      setTopics(tMap);
  ...
  const loadSPMSyllabus = async () => {
    ...
    try {
       // Add topics
       const spmTopics = [
         { id: uuidv4(), title: 'Newtonian Mechanics', subject: 'Physics', priority: 'emergency', masteryLevel: 0, createdAt: serverTimestamp(), updatedAt: serverTimestamp() },
         { id: uuidv4(), title: 'Organic Chemistry', subject: 'Chemistry', priority: 'emergency', masteryLevel: 0, createdAt: serverTimestamp(), updatedAt: serverTimestamp() },
         { id: uuidv4(), title: 'Cell Biology', subject: 'Biology', priority: 'normal', masteryLevel: 0, createdAt: serverTimestamp(), updatedAt: serverTimestamp() },
         { id: uuidv4(), title: 'Calculus Differentiation', subject: 'Add Maths', priority: 'emergency', masteryLevel: 0, createdAt: serverTimestamp(), updatedAt: serverTimestamp() },
       ];
       ...
       const newBlocks: ScheduleBlock[] = [
          { id: uuidv4(), userId: user.uid, title: spmTopics[0].title, topicId: spmTopics[0].id, day: mondayStr, startTime: '15:00', endTime: '17:00', status: 'upcoming', createdAt: serverTimestamp() as any, updatedAt: serverTimestamp() as any },
          { id: uuidv4(), userId: user.uid, title: spmTopics[1].title, topicId: spmTopics[1].id, day: mondayStr, startTime: '19:00', endTime: '21:00', status: 'upcoming', createdAt: serverTimestamp() as any, updatedAt: serverTimestamp() as any },
          { id: uuidv4(), userId: user.uid, title: spmTopics[2].title, topicId: spmTopics[2].id, day: tuesdayStr, startTime: '15:00', endTime: '17:00', status: 'upcoming', createdAt: serverTimestamp() as any, updatedAt: serverTimestamp() as any },
          { id: uuidv4(), userId: user.uid, title: spmTopics[3].title, topicId: spmTopics[3].id, day: tuesdayStr, startTime: '20:00', endTime: '22:00', status: 'upcoming', createdAt: serverTimestamp() as any, updatedAt: serverTimestamp() as any },
       ];
  ...
  const handleAddBlock = async () => {
     if (!user || !newBlockTopicId || !newBlockDate || !newBlockStartTime || !newBlockEndTime) return;
     try {
  ...
  const getWeekDays = () => {
    ...
    if (viewMode === 'Day') {
        ...
        for (let i = 0; i < 35; i++) { // 5 weeks layout
  ...
  const findNextAvailableSlot = (blocks: ScheduleBlock[], startDate: Date, delayDays: number = 1, searchWindow: number = 3): { day: string; start: string } | null => {
    ...
    for (let i = 0; i < searchWindow; i++) { 
      ...
      const dayBlocks = blocks.filter(b => b.day === dateString).sort((a, b) => a.startTime.localeCompare(b.startTime));

      for (let hour = STUDY_HOURS.start; hour <= STUDY_HOURS.end; hour += 2) {
        const potentialStart = `${hour.toString().padStart(2, '0')}:00`;
        let nextHour = hour + 2;
        const potentialEnd = nextHour === 24 ? '23:59' : `${nextHour.toString().padStart(2, '0')}:00`;

        const isOverlap = dayBlocks.some(b => (potentialStart < b.endTime && potentialEnd > b.startTime));
  ...
  const [selectedFeedback, setSelectedFeedback] = useState<{ option: string, isCorrect: boolean } | null>(null);
  ...
  const submitAnswer = async (selectedAnswer: string) => {
    ...
    try {
      ...
      const topicRef = doc(db, 'users', user.uid, 'topics', currentQuiz.topicId);
      ...
      const newMastery = isCorrect ? Math.min(100, currMastery + 20) : Math.max(0, currMastery - 10);
      const newFailedAttempts = isCorrect ? Math.max(0, failedAttempts - 1) : failedAttempts + 1;
      ...
      const blockRef = doc(db, 'users', user.uid, 'scheduleBlocks', currentQuiz.id);
      ...
      if (!isCorrect) {
        ...
        if (nextSlot) {
          ...
          const newBlock: ScheduleBlock = {
            ...
            endTime: parseInt(nextSlot.start.split(':')[0]) + 2 === 24 ? '23:59' : `${(parseInt(nextSlot.start.split(':')[0]) + 2).toString().padStart(2, '0')}:00`,
            status: 'upcoming',
            isReview: true
          };

          await setDoc(doc(db, 'users', user.uid, 'scheduleBlocks', newBlockId), {
             ...newBlock,
             createdAt: serverTimestamp(),
             updatedAt: serverTimestamp()
          });
          ...
          newBlocks.push(newBlock);
           ...
           alert("No available slots found within the next 72 hours to reschedule this topic. Please free up some time or manually add a review block.");
        }
      }
  ...
  if (!user) {
    return (
      <div className="flex-1 p-8 flex items-center justify-center flex-col text-center">
          ...
          <p className="text-neutral-500 dark:text-neutral-400">Please sign in with Google from the sidebar to view your learning schedule.</p>
      </div>
    );
  }
    ...
    <div className="flex-1 overflow-x-hidden overflow-y-auto p-4 md:p-8 bg-neutral-50 dark:bg-[#0c0c0e]">
      <header className="mb-8 flex flex-col items-start justify-between gap-4">
        <div>
          ...
          <p className="text-neutral-500 dark:text-neutral-400 text-sm mt-1">Review your upcoming Lumina study sessions.</p>
        </div>
        <div className="flex flex-wrap w-full items-center gap-4 justify-between">
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto">
            ...
            <div className="flex items-center gap-2">
               <button onClick={() => setWeekOffset(w => w - 1)} className="p-2 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg hover:bg-neutral-50 dark:hover:bg-neutral-800 text-neutral-900 dark:text-white transition-colors">
                  <ChevronLeft className="w-5 h-5" />
               </button>
               <button onClick={() => setWeekOffset(0)} className="px-4 py-2 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg hover:bg-neutral-50 dark:hover:bg-neutral-800 text-sm font-medium text-neutral-900 dark:text-white transition-colors">
                  Today
               </button>
               <button onClick={() => setWeekOffset(w => w + 1)} className="p-2 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg hover:bg-neutral-50 dark:hover:bg-neutral-800 text-neutral-900 dark:text-white transition-colors">
        ...
        <div className="min-w-[700px] md:min-w-[900px] flex">
          ...
          {(viewMode === 'Day' || viewMode === 'Week') && (
            <div className="w-16 border-r border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900/30 flex flex-col pt-[72px]">
              {Array.from({ length: STUDY_HOURS.end - STUDY_HOURS.start + 1 }).map((_, i) => (
                <div key={i} className="h-[60px] text-[10px] font-bold text-neutral-400 text-right pr-2 -mt-2">
          ...
          <div className="flex-1 flex flex-col">
            {/* Header */}
            <div className={`grid ${viewMode === 'Day' ? 'grid-cols-1' : viewMode === 'Week' ? 'grid-cols-7' : 'grid-cols-7'} border-b border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900/30`}>
              {(viewMode === 'Month' ? currentWeekDays.slice(0, 7) : currentWeekDays).map((date, i) => {
                const localTodayDate = new Date(`${getLocalTodayDateString()}T00:00:00`);
                localTodayDate.setHours(0, 0, 0, 0);
                const isToday = localTodayDate.toDateString() === date.toDateString();
                return (
                  <div key={i} className={`py-4 text-center border-r border-neutral-200 dark:border-neutral-800 last:border-r-0 ${isToday && viewMode !== 'Month' ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''}`}>
                    <span className={`text-[10px] font-bold uppercase tracking-widest ${isToday && viewMode !== 'Month' ? 'text-blue-600 dark:text-blue-500' : 'text-neutral-500'}`}>
                      {date.toLocaleDateString('en-US', { weekday: 'short' })}
                    </span>
                    {viewMode !== 'Month' && (
                      <div className={`mt-1 text-lg font-medium ${isToday ? 'text-blue-600 dark:text-blue-400' : 'text-neutral-900 dark:text-white'}`}>
                        {date.getDate()}
                      </div>
                    )}
                  </div>
            ...
            <div 
               ...
               style={{ minHeight: viewMode === 'Month' ? '600px' : `${(STUDY_HOURS.end - STUDY_HOURS.start + 1) * 60}px` }}
            >
              {/* Horizontal background lines for Time */}
              {(viewMode === 'Day' || viewMode === 'Week') && (
                 <div className="absolute inset-0 pointer-events-none flex flex-col">
                    {Array.from({ length: STUDY_HOURS.end - STUDY_HOURS.start + 1 }).map((_, i) => (
                      <div key={i} className="h-[60px] border-b border-neutral-100 dark:border-neutral-800/50 w-full shrink-0" />
                    ))}
                 </div>
              )}
              ...
              {currentWeekDays.map((date, i) => {
                ...
                return (
                  ...
                  >
                    {isMonthView && (
                      <div className={`text-sm font-medium mb-2 ${isToday ? 'bg-blue-600 text-white w-6 h-6 rounded-full flex items-center justify-center' : 'text-neutral-500'}`}>
                        {date.getDate()}
                      </div>
                    )}
                    {schedule.filter(b => b.day === dateString).map(block => {
                      const startHour = parseInt(block.startTime.split(':')[0]);
                      const startMin = parseInt(block.startTime.split(':')[1] || '0');
                      let endHour = parseInt(block.endTime.split(':')[0]);
                      let endMin = parseInt(block.endTime.split(':')[1] || '0');
                      if (endMin === 59) { endHour += 1; endMin = 0; } // Hack for 23:59
                      
                      const topPx = isMonthView ? 0 : Math.max(0, ((startHour - STUDY_HOURS.start) * 60) + startMin);
                      const heightPx = isMonthView 
                           ? 'auto' 
                           : Math.max(30, ((endHour - startHour) * 60) + (endMin - startMin));
                      ...
                      >
                        ...
                        {!isMonthView && (
                          ...
                          <button 
                            onClick={(e) => { e.stopPropagation(); openQuiz(block); }}
                    ...
                    {isMonthView && schedule.filter(b => b.day === dateString).length === 0 && (
            ...
            >
              <div className="flex justify-between items-start mb-4">
                 <div>
                    <span className={`text-xs font-bold uppercase tracking-widest ${
                      selectedBlockDetail.status === 'mastered' ? 'text-emerald-600' : 
                      selectedBlockDetail.status === 'failed' ? 'text-rose-600' : 'text-blue-600'
                    }`}>
                      {selectedBlockDetail.status}
                    </span>
                    <h2 className="text-xl font-semibold text-neutral-900 dark:text-white mt-1">{selectedBlockDetail.title}</h2>
                    <p className="text-sm text-neutral-500 dark:text-neutral-400 mt-1">{selectedBlockDetail.day} • {selectedBlockDetail.startTime} - {selectedBlockDetail.endTime}</p>
                 </div>
                 <button onClick={() => setSelectedBlockDetail(null)} className="p-1.5 bg-neutral-100 dark:bg-neutral-800 text-neutral-500 rounded-lg hover:bg-neutral-200 dark:hover:bg-neutral-700">
                   <X className="w-4 h-4" />
                 </button>
              </div>
              ...
              <div className="space-y-4">
                 <div className="p-4 bg-neutral-50 dark:bg-neutral-900/50 rounded-xl border border-neutral-200 dark:border-neutral-800">
                   <h3 className="text-xs font-bold text-neutral-500 uppercase tracking-widest mb-2">Topic Information</h3>
                   {topics[selectedBlockDetail.topicId] ? (
                     <>
                        <div className="mb-3">
                           <div className="text-sm text-neutral-900 dark:text-neutral-200 mb-1">
                             <span className="font-medium">Subject:</span> {topics[selectedBlockDetail.topicId].subject}
                           </div>
                           <div className="flex items-center gap-2">
                             <span className="text-sm font-medium text-neutral-900 dark:text-neutral-200">Mastery:</span>
                             <div className="flex-1 h-2 bg-neutral-200 dark:bg-neutral-800 rounded-full overflow-hidden">
                               <div 
                                 className="h-full bg-blue-500" 
                                 style={{ width: `${topics[selectedBlockDetail.topicId].masteryLevel}%` }}
                               />
                             </div>
                             <span className="text-xs font-bold text-blue-600 dark:text-blue-500">{topics[selectedBlockDetail.topicId].masteryLevel}%</span>
                           </div>
                        </div>
                        {topics[selectedBlockDetail.topicId].notes && (
                           <div>
                             <span className="text-sm font-medium text-neutral-900 dark:text-neutral-200">Notes:</span>
                             <p className="mt-1 text-sm text-neutral-600 dark:text-neutral-400 whitespace-pre-wrap">{topics[selectedBlockDetail.topicId].notes}</p>
                           </div>
                        )}
                     </>
                   ) : (
                     <p className="text-sm text-neutral-500">Topic details no longer available.</p>
                   )}
                 </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="flex justify-between items-center mb-6">
                 ...
                 <button onClick={() => setShowAddModal(false)} className="text-neutral-400 hover:text-neutral-900 dark:hover:text-white">
                   <X className="w-5 h-5" />
                 </button>
              </div>
              ...
              <div className="space-y-4">
                 <div>
                    <label className="block text-xs font-bold text-neutral-500 uppercase tracking-widest mb-1.5">Topic</label>
                    <select 
                 ...
                 <div className="grid grid-cols-2 gap-4">
                   <div>
                      <label className="block text-xs font-bold text-neutral-500 uppercase tracking-widest mb-1.5">Start Time</label>
                      <input 
                        type="time"
                   ...
                   <div>
                      <label className="block text-xs font-bold text-neutral-500 uppercase tracking-widest mb-1.5">End Time</label>
                      <input 
                        type="time"
                 ...
                 <button 
                   ...
                   className="w-full mt-4 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-medium transition-all active:scale-95 shadow-md"
                 >
                   Save Session
                 </button>
              </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <h2 className="text-xl font-semibold text-neutral-900 dark:text-white mb-4 leading-tight">
                Review: <span className="text-blue-600 dark:text-blue-400">{currentQuiz.title.split(':')[0]}</span>
              </h2>
