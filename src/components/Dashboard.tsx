...
export const Dashboard: React.FC<{ onNavigateToCalendar: () => void }> = ({ onNavigateToCalendar }) => {
  ...
  const progressionData = useMemo(() => {
     let curMastery = 0;
     const points: any[] = [];
     const logsForTopic = selectedTopicId === 'all' 
         ? quizLogs
         : quizLogs.filter(l => l.topicId === selectedTopicId);
     
     logsForTopic.forEach(log => {
         if (log.isCorrect) {
             curMastery = Math.min(100, curMastery + (selectedTopicId === 'all' ? 2 : 20));
         } else {
             curMastery = Math.max(0, curMastery - (selectedTopicId === 'all' ? 1 : 10));
         }
         let dateStr = "Unknown";
    ...
    const fetchData = async () => {
      try {
        ...
        if (userSnap.exists()) {
          ...
          const tz = pData.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC';
          ...
          if (pData.lastStreakDate && pData.lastStreakDate !== todayStr) {
             ...
             if (pData.lastStreakDate !== yesterdayStr && newStreak > 0) {
                 ...
                 if (pData.activePet) {
                     ...
                     alert("Oh no! You lost your streak. Your pet lost 20% happiness.");
                 }
                 needsUpdate = true;
             }
          }
        ...
        setQuizLogs(logsSnap.docs.map(d => ({ id: d.id, ...d.data() } as QuizLog)));
  ...
  const totalMastery = topics.length > 0 ? topics.reduce((acc, t) => acc + t.masteryLevel, 0) / topics.length : 0;
  ...
  return (
    <div className="space-y-8 flex-1 p-6 md:p-8 overflow-y-auto">
      <header className="mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-neutral-900 dark:text-white tracking-tight">{t('dashboard')}</h1>
          <p className="text-neutral-500 dark:text-neutral-400 text-sm mt-1">Welcome back. Lumina brings clarity to your learning journey.</p>
        </div>
        <button onClick={onNavigateToCalendar} className="px-4 py-2 bg-yellow-400 hover:bg-yellow-500 text-yellow-900 rounded-lg text-sm font-bold shadow-md transition-colors w-full sm:w-auto text-center flex justify-center items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 shadow-sm rounded-2xl p-6 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-neutral-900 dark:text-white">{profile?.streak || 0} Days</p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 shadow-sm rounded-2xl p-6 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-neutral-500 uppercase font-bold tracking-widest">Global Mastery</p>
            <p className="text-2xl font-bold text-neutral-900 dark:text-white">{Math.round(totalMastery)}%</p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 shadow-sm rounded-2xl p-6 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-neutral-900 dark:text-white">{topics.length}</p>
          </div>
        </div>
      </div>
            ...
            {topics.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%" minHeight={1} minWidth={1}>
                <AreaChart data={data}>
                  ...
                  />
                  <Area type="monotone" dataKey="retention" stroke="#3b82f6" strokeWidth={3} fillOpacity={1} fill="url(#colorRetention)" />
                </AreaChart>
              </ResponsiveContainer>
            )}
          </div>
      ...
      <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 shadow-sm rounded-2xl p-6">
        <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h3 className="text-lg font-medium text-neutral-900 dark:text-white">Mastery Progression</h3>
            <p className="text-xs text-neutral-500 dark:text-neutral-400">Track how your mastery has evolved over time.</p>
          </div>
          <select
        ...
        <div className="h-72 w-full mt-4">
          {progressionData.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center border border-dashed border-neutral-200 dark:border-neutral-800 rounded-xl">
              <p className="text-neutral-500 dark:text-neutral-400 mb-2">No quiz data yet.</p>
            ...
            <ResponsiveContainer width="100%" height="100%" minHeight={1} minWidth={1}>
              <LineChart data={progressionData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                ...
                <Tooltip 
                  ...
                  itemStyle={{ color: '#e5e5e5' }}
                />
                <Line type="monotone" dataKey="mastery" stroke="#8b5cf6" strokeWidth={3} dot={{ r: 4, fill: '#8b5cf6', strokeWidth: 0 }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
